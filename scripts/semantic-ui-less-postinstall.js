...
const SITE_ROOT = "src/my-semantic-theme";
        ...
        fs.access(file, fs.F_OK, err => {
          if (err) {
            ...
            let content = fs.readFileSync(
              SEMANTIC_ROOT + "/theme.config.example",
              "utf8"
            );
      ...
      new Promise((resolve, reject) => {
        ...
        fs.access(file, fs.F_OK, err => {
          if (err) {
            if (err.code !== "ENOENT") return reject(err);
            console.log(
              "  Creating theme.config file inside of semantic-ui-less"
            );
            fs.writeFileSync(
              file,
